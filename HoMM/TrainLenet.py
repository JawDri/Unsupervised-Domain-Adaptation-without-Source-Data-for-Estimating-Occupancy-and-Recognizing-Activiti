...
num_features = Source_train.shape[1] - 1
SourceData = Source_train.drop('activity', axis = 1)
SourceLabel = Source_train.activity.astype(np.int)
Target_train = pd.read_csv("./data/Target_train.csv")
TargetData = Target_train.drop('activity', axis = 1)
TargetLabel = Target_train.activity.astype(np.int)
Target_test = pd.read_csv("./data/Target_test.csv")
TestData = Target_test.drop('activity', axis = 1)
TestLabel = Target_test.activity

SourceData = SourceData.values.reshape(len(SourceData), 1, num_features,1)
TargetData = TargetData.values.reshape(len(TargetData), 1, num_features,1)
TestData = TestData.values.reshape(len(TestData), 1, num_features,1)
class Train():
    def __init__(self,class_num,batch_size,iters,learning_rate,keep_prob,param):
        ...
        self.SourceData, self.SourceLabel=SourceData, SourceLabel
        self.TargetData, self.TargetLabel=TargetData, TargetLabel
        ...
        #######################################################################################
        self.source_image = tf.placeholder(tf.float32, shape=[self.BatchSize, 1,num_features,1],name="source_image")
        self.source_label = tf.placeholder(tf.float32, shape=[self.BatchSize, self.ClassNum],name="source_label")
        self.target_image = tf.placeholder(tf.float32, shape=[self.BatchSize, 1, num_features,1],name="target_image")
    ...
    def TrainNet(self):
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=tf.GPUOptions(allow_growth=True))) as sess:
            ...
            sess.run(init)
            self.SourceLabel=sess.run(tf.one_hot(self.SourceLabel,self.ClassNum))
            self.TestLabel=sess.run(tf.one_hot(self.TestLabel,self.ClassNum))
            true_num = 0.0
    ...
    def Test(self,sess):
        ...
        for i in range (num):
            ...
            f1score+= f1_score(true_label, prediction, average = 'weighted')
            fscore+= score(true_label, prediction)[2]
        accuracy=true_num / total_num
        f1ScoreW = f1score / num
        fscoreW = fscore / num
        print("###########  Test Accuracy={} ##########".format(accuracy))
        print("###########  Test F1-Score={} ##########".format(f1ScoreW))
        print("###########  Test F1-Score-labels={} ##########".format(fscoreW))
...
def main():
    ...
    Runer=Train(class_num=3,batch_size=32,iters=40000,learning_rate=0.0001,keep_prob=1,param=param)
    Runer.TrainNet()
