...
def APM_init_update(feature_extractor, classifier_t):
    ...
    for img in target_train_dl:
        im_target_lbcorr = img['X']
        label_target_lbcorr = img['Y']
        ...
        for cls in range(len(source_classes)):
            ...
            filtered_feat = torch.gather(fc1_lbcorr.cpu(), dim=0, index=list_loc.unsqueeze(1).repeat(1, 256))
